...
#define vector4Test_imp(type, cast, name, eigenType, tab)                                                              \
  {                                                                                                                    \
    ...
    mipp::Reg<type> res4M_old = cast(res4_old);                                                                        \
                                                                                                                       \
    hasFailed |= printWhenDiff((!mipp::testz(res1M != res1M_old) || !mipp::testz(res2M != res2M_old) ||                \
                                !mipp::testz(res3M != res3M_old) || !mipp::testz(res4M != res4M_old)),                 \
                               #name "<" #eigenType ">(" + to_sting(tab) + ", P*, P*, P*, P*)",                        \
                               to_sting(res1M) + to_sting(res2M) + to_sting(res3M) + to_sting(res4M),                  \
                               to_sting(res1M_old) + to_sting(res2M_old) + to_sting(res3M_old) + to_sting(res4M_old)); \
  }
